...
class GiveawaysManager extends EventEmitter {
    ...
    constructor(client, options) {
        super();
        if (!client) throw new Error('Client est une option requise.');
        /**
         * The Discord Client
         * @type {Discord.Client}
         */
    ...
    start(channel, options) {
        return new Promise(async (resolve, reject) => {
            if (!this.ready) {
                return reject('Le manager n\'est pas encore prêt.');
            }
            if (!options.messages) {
                options.messages = defaultGiveawayMessages;
            }
            if (!channel || !channel.id) {
                return reject(`channel n'est pas un guildchannel (channel de serveur) valide. (val=${channel})`);
            }
            if (!options.time || isNaN(options.time)) {
                return reject(`options.time n'est pas un nombre. (val=${options.time})`);
            }
            if (!options.prize) {
                return reject(`options.prize n'est pas une chaîne de caractères. (val=${options.prize})`);
            }
            if (!options.winnerCount || isNaN(options.winnerCount)) {
                return reject(`options.winnerCount n'est pas un nombre. (val=${options.winnerCount})`);
            }
    ...
    reroll(messageID, options = {}) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!giveawayData) {
                return reject('Pas de concours trouvé avec l\'ID ' + messageID + '.');
            }
    ...
    delete(messageID, doNotDeleteMessage) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!giveaway.channel) {
                return reject('Incapable d\'avoir le channel du concours avec l\'ID (du message) ' + giveaway.messageID + '.');
            }
        ...
        if (!storageExists) {
            ...
            try {
                ...
                if (Array.isArray(giveaways)) {
                    ...
                    throw new SyntaxError('Le fichier de stockage n\'est pas correctement formaté (giveaway ("concours") n\'est pas un tableau (array)).');
                }
            } catch (e) {
                if (e.message === 'Unexpected end of JSON input') {
                    throw new SyntaxError('Le fichier de stockage n\'est pas correctement formaté (Fin inattendue de la contribution de JSON).');
                } else {
                    throw e;
                }
            }
